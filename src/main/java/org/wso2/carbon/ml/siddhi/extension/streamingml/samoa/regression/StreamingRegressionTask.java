...
public class StreamingRegressionTask implements Task, Configurable {
    ...
    public ClassOption streamTrainOption = new ClassOption("trainStream", 's', "Stream to learn from.",
            InstanceStream.class, StreamingRegressionStream.class.getName());
    ...
    public IntOption timeLimitOption = new IntOption("timeLimit", 't', "Maximum number of seconds to test/train for " +
            "(-1 = no limit).", -1, -1, Integer.MAX_VALUE);
    public IntOption sampleFrequencyOption = new IntOption("sampleFrequency", 'f', "How many instances between samples " +
            "of the learning performance.", 100000, 0, Integer.MAX_VALUE);
    ...
    public IntOption batchDelayOption = new IntOption("delayBatchSize", 'b', "The delay batch size: delay " +
            "of x milliseconds after each batch ", 1, 1, Integer.MAX_VALUE);
    ...
    public void init() {
        ...
        if (streamTrain instanceof StreamingRegressionStream) {
            ...
            throw new ExecutionPlanRuntimeException("Check stream: Stream is not a StreamingRegressionStream");
        }
